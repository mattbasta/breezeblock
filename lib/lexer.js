var TOKENS = [
    ...
    [/of[^\w]/, 'of'], // for use with {% loop x of y %}
...
Lexer.prototype.readUntil = function readUntil(tokens) {
    if (this.peeked && tokens.some(function(tok) {return this.peeked.type === tok;}, this)) {
        return '';
    }
    ...
    var remainingData = this.remainingData;
    var prepended = this.peeked ? this.peeked.text.length : 0;
    if (prepended) {
        remainingData = this.peeked.text + remainingData;
        this.peeked = null;
    }
    ...
    for (var i = 0; i < tokens.length; i++) {
        ...
        match = tok[0].exec(remainingData);
        if (!match) {
            continue;
        }
    ...
    var output = remainingData.substr(0, lastIndex);
    this.remainingData = this.remainingData.substr(lastIndex - prepended);
    return output;
};

/**
 * Returns whether the lexer is at the end of the file
 * @return {bool}
 */
Lexer.prototype.isEOF = function isEOF() {
    return !this.peeked && !this.remainingData;
};
