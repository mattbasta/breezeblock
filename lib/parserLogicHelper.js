...
exports.acceptVariable = function acceptVariable(lex) {
    ...
    var out = exports.assertInnerExpression(lex, unsafe);
    if (unsafe) {
        lex.assert('}}}');
    } else {
        lex.assert('}}');
 ...
 * @param  {object} lex The lexer
 * @param  {bool} unsafe Whether the value should be output unescaped
 * @return {LogicVariable|LogicMember|LogicSubscript}
 */
exports.assertInnerExpression = function assertInnerExpression(lex, unsafe) {
    var temp = exports.acceptInnerLiteral(lex);
    if (temp) {
        return temp;
    }

    var expr = new nodes.LogicVariable(unsafe);
    expr.parse(lex);

    function parseMember(expr) {
        if (lex.accept('.')) {
            var mem = new nodes.LogicMember(expr, unsafe);
            mem.parse(lex);
            return parseMember(mem);
        }
        if (lex.accept('[')) {
            var mem = new nodes.LogicSubscript(expr, unsafe);
            mem.parse(lex);
            return parseMember(mem);
        }
