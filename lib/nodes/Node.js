...
Node.prototype.parse = function(lex) {
    ...
    function parseAttributeSection() {
        var temp;
        while (lex.peek().type !== '>') {

            temp = parseVariableAttribute() ||
                   parseStaticAttribute();

            if (temp) {
                me.attributes.push(temp);
                continue;
            }
    ...
    function parseVariableAttribute() {
...
Node.prototype.asDOMGenerator = function asDOMGenerator(ctx) {
    ...
    } else if (this.contents.every(function(x) {
        return x instanceof nodes.TextNode ||
               x instanceof nodes.LogicVariable;
    })) {
