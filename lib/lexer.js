...
Lexer.prototype.next = function next() {
    ...
    if (!token) {
        if (!this.remainingData.trim()) return 'EOF';
        throw new SyntaxError('Unknown token at line ' + currentLine + ' near "' + this.remainingData.substr(0, 20) + '"');
    }
    return token;
};
...
Lexer.prototype.assert = function assert(tokenType) {
    ...
    if (next === 'EOF') {
        ...
        throw new SyntaxError('Unexpected token "' + next.type + '", expected "' + tokenType + '"');
    }
    return next;
};
